...
cd ${PROJECT_DIR}
commitCount=`"$git" rev-list master --count`
bundleVersion=$(($commitCount + ${VERSION_OFFSET}))
versionString=`"$git" describe --dirty --match [0-9]* --tags --abbrev=0`
versionString="$versionString build $bundleVersion"
